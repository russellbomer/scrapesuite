...
Today's work focused on **framework detection integration** and **network resilience improvements** for the ScrapeSuite wizard.
...
### Core Modules
- `scrapesuite/http.py` - Timeout improvements (15s→30s, better retry logic)
- `scrapesuite/wizard.py` - HTML paste fallback, error handling
- `scrapesuite/inspector.py` - Framework detection integration, pattern boosting
- `scrapesuite/framework_profiles.py` - Added `is_framework_pattern()` helper

### New Modules
- `scrapesuite/detection_strategies.py` - Advanced detection (table headers, semantic HTML)
