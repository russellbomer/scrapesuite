...
from scrapesuite.lib.schemas import ExtractionSchema
...
def preview_extraction(html: str, schema: ExtractionSchema, limit: int = 5) -> list[dict[str, Any]]:
    ...
    try:
        item_elements = soup.select(schema.item_selector)
        ...
        for field_name, field_schema in schema.fields.items():
            try:
                ...
                if field_schema.multiple:
                    elements = item_elem.select(field_schema.selector)
                else:
                    elements = item_elem.select(field_schema.selector)
...
def _extract_value(element: Any, attribute: str | None) -> str | None:
