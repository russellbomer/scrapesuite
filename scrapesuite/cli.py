...
app = typer.Typer(
    help=(
        "ScrapeSuite - Web scraping toolkit with offline testing, "
...
def check_robots(
    ...
    user_agent: str = typer.Option("ScrapeSuite", "--user-agent", "-ua", help="User-Agent string"),
) -> None:
    """
    ...
    Examples:
        ...
        $ python -m scrapesuite.cli check-robots https://reddit.com/r/python -ua "MyBot"
    """
    try:
        cache = RobotsCache()
        parsed = urlparse(url)
...
def failed(
    ...
    Examples:
        ...
        $ python -m scrapesuite.cli failed  # Show all failed URLs
    """
    try:
        conn = open_db(db)
...
def inspect(
    ...
    Examples:
        $ python -m scrapesuite.cli inspect jobs/fda.yml
        $ python -m scrapesuite.cli inspect examples/jobs/nws.yml
    """
    try:
        job_dict = load_yaml(job_yaml)
